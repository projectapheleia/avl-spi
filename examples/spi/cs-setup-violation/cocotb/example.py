...
async def test(dut):
    """
    Example SPI chip select setup violation
        - Interface CS_SETUP 3, CS_HOLD 2 and CS_GAP 4 clk cycles
        - Master cs_setup overridden to 1 clk cycle
        - Expected to end with $fatal from the interface cs setup check
    ...
    avl.Factory.set_variable("*.agent.mdrv.cs_setup", 1)
