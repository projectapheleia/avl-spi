...
async def test(dut):
    """
    Example SPI chip select hold violation
        - Interface CS_SETUP 3, CS_HOLD 2 and CS_GAP 4 clk cycles
        - Master cs_hold overridden to 1 clk cycle
        - Expected to end with $fatal from the interface cs hold check
    ...
    avl.Factory.set_variable("*.agent.mdrv.cs_hold", 1)
